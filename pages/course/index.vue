        ...
        <div class="js-wrap">
          ...
          <section class="fl">
            ...
            <ol class="js-tap clearfix">
              <li v-show="searchObj.type === '1'" :class="{'current bg-green' : searchObj.buyCountSort}">
                <a title="销量" href="javascript:void(0);" @click="searchBuyCount(2)">销量
                  <i v-if="searchObj.buyCountSort">↓</i>
                </a>
              </li>
              <li v-show="!searchObj.type || searchObj.type === '2'" :class="{'current bg-green' : searchObj.buyCountSort}">
                <a title="销量" href="javascript:void(0);" @click="searchBuyCount(1)">销量
                  <i v-if="searchObj.buyCountSort">↑</i>
                </a>
              </li>
              <li v-show="searchObj.type === '1'" :class="{'current bg-green' : searchObj.gmtCreateSort}">
                <a title="最新" href="javascript:void(0);" @click="searchGmtCreate(2)">最新
                  <i v-if="searchObj.gmtCreateSort">↓</i>
                </a>
              </li>
              <li v-show="!searchObj.type || searchObj.type === '2'" :class="{'current bg-green' : searchObj.gmtCreateSort}">
                <a title="最新" href="javascript:void(0);" @click="searchGmtCreate(1)">最新
                  <i v-if="searchObj.gmtCreateSort">↑</i>
                </a>
              </li>
              <li v-show="searchObj.type === '1'" :class="{'current bg-green' : searchObj.priceSort}">
                <a title="价格" href="javascript:void(0);" @click="searchPrice(2)">价格
                  <i v-if="searchObj.priceSort">↓</i>
                </a>
              </li>
              <li v-show="!searchObj.type || searchObj.type === '2'" :class="{'current bg-green' : searchObj.priceSort}">
                <a title="价格" href="javascript:void(0);" @click="searchPrice(1)">价格
                  <i v-if="searchObj.priceSort">↑</i>
                </a>
              </li>
            </ol>
...
export default {
  async asyncData(page) {
    ...
    // 从url地址栏仲获取查询参数, 对页面中响应部分的内容进行高亮显示
    const searchObj = {}
    ...
    searchObj.type = query.type || '' // 1: 升序, 2: 倒序
  ...
  methods: {
    ...
    searchSubjectLevelOne(subjectParentId) {
      // subjectParentId.length > 0 ? window.location = 'course?subjectParentId=' + subjectParentId
    ...
    searchSubjectLevelTwo(subjectId) {
      // subjectId.length > 0 ? window.location = 'course?subjectParentId=' + this.searchObj.subjectParentId + '&subjectId=' + subjectId
      //   : this.searchSubjectLevelOne(this.searchObj.subjectParentId)
